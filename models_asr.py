...
class EncoderConnectorLmWithPretrainedLm(nn.Module):
    ...
    def forward(self, x, is_text = False, output_hidden_states=False, last_layer_MSE = False):
        ...
        if is_text and self.text_encoder is None:
            text_embedding = self.lm.get_input_embeddings()(text)
            context_embedding = self.lm.get_input_embeddings()(context)
            context_connector_output = context_embedding
            connector_output_r = shift_batch_right(context_connector_output, context_lengths)
            
            context_connector_lengths = context_lengths
        ...
        text_mask = torch.ones(text_embedding.shape[0], text_embedding.shape[1], device=text_embedding.device, dtype=torch.bool)
    ...
    def generate_dialog_single_turn(self, x, max_len, bos_token):
        ...
        remaining = remaining.to(connector_output.device)
        connector_mask = torch.cat((remaining, connector_mask), dim=1)
