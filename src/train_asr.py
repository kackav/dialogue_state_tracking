...
def main():
    ...
    if args.resume:
        ...
        if args.train_encoder:
            if re.match(r"^nvidia/parakeet-ctc", args.encoder_model_name):
                encoder = models.ParakeetWrapper(args.encoder_model_name)
                connector = models.Connector(encoder.encoder.config.hidden_size, lm.config.hidden_size, num_heads = 4, ff_size = 4*encoder.encoder.config.hidden_size, num_connector_layers = args.connector_layers, kernel_sizes = (10,), strides = (2,))
            else:
    ...
    dset_valid = data.load_from_config('validation', datasets_config, hf_dataset_path) # dictonary ["commonvoice": ..., "librispeech": ... etc]
    val_subsets = {}
    for k, v in dset_valid.items():
        val_subsets[k] = data.AudioDatasetHF({k:v}, tokenizer, bos_token)
    ...
    # train dataset
    dset_train_s = data.load_from_config('train', datasets_config, hf_dataset_path)
    train_asr_data = data.AudioDatasetHF(dset_train_s, tokenizer, bos_token)
    if args.text_input:
        dset_train_t = data.load_from_config('text_train', datasets_config, hf_dataset_path)
        train_text_data = data.TextDatasetHF(dset_train_t, tokenizer, bos_token)
    ...
    # training dataloader
    collate_fn_asr = data.CollateFnSpeech(tokenizer)
    ...
    if args.text_input:
        collate_fn_text = data.CollateFnText(tokenizer) #functools.partial(data.collate_fn_text, tokenizer=tokenizer)
                    ...
                    with torch.autocast(enabled = True, device_type = "cuda", dtype= torch.bfloat16):
                        ...
                        if args.text_input:
                            if args.train_with_asr_text:
                                x_all_t = data.collate_all_text(x_s_clone, x_t, tokenizer = tokenizer)
                                ...
                                if args.mse_loss:
                                    ...
                                    loss_t = loss_t + args.mse_weight*mse_loss
