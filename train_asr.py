...
def main():
    ...
    lm = transformers.AutoModelForCausalLM.from_pretrained(lm_model_name, trust_remote_code=True, torch_dtype=torch.bfloat16, attn_implementation='flash_attention_2', device_map=accelerator.device, **quantization_kwargs)
    ...
    if args.pretrained_dir is not None:
        ...
        if os.path.isfile(os.path.join(args.pretrained_dir, 'TextEncoder_model.pt')):
            ...
            text_encoder = models_asr.TextEncoder(args.mask_rate, vocab_size=None, dim_input = args.text_encoder_dim, dim_output = encoder.encoder.config.hidden_size, num_heads=4, ff_size = 4*(args.text_encoder_dim), pad_token=tokenizer.pad_token_id, dim_lm_embeddings=lm_hidden_size, num_layers = args.text_encoder_layers)
    else:
        print("initialize model parts")
        encoder = models_asr.WavLMWrapper(args.encoder_model_name)
        connector = models_asr.Connector(encoder.encoder.config.hidden_size, lm_hidden_size, num_heads = 4, ff_size = 4*encoder.encoder.config.hidden_size, num_connector_layers = args.connector_layers)
