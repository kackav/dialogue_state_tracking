...
def load_dial_from_config(ds_type, config_datasets, hf_dataset_path, tokenizer=None, accelerator=None, do_filter=False):
    loaded_datasets = {}
    max_audio_len=30
    min_audio_len=0.6
    max_text_len=200
    ...
    for k,v in config_datasets[ds_type].items():
        ...
        if "path_to_subsets" in v:
            ...
            for ds in os.listdir(dataset_path):
                ...
                if do_filter:
                    loaded_dataset = loaded_dataset.filter(functools.partial(filter_long_text, tokenizer=tokenizer, max_text_len=max_text_len), num_proc=4)   
                    loaded_dataset = loaded_dataset.filter(functools.partial(filter_short_text, tokenizer=tokenizer, min_text_len=min_text_len), num_proc=4)   
            ...
            if 'audio_len' in loaded_datasets[k].features:
                if do_filter:
                    loaded_datasets[k] = loaded_datasets[k].filter(lambda item: item['audio_len']<(max_audio_len*16_000), num_proc=4)
                    loaded_datasets[k] = loaded_datasets[k].filter(lambda item: item['audio_len']>(min_audio_len*16_000), num_proc=4)
            if do_filter:
                loaded_datasets[k] = loaded_datasets[k].filter(functools.partial(filter_long_text, tokenizer=tokenizer, max_text_len=max_text_len), num_proc=4)

                loaded_datasets[k] = loaded_datasets[k].filter(functools.partial(filter_short_text, tokenizer=tokenizer, min_text_len=min_text_len), num_proc=4)   
        else:
            ...
            if 'audio' in loaded_datasets[k].features:
                if do_filter:
                    loaded_datasets[k] = loaded_datasets[k].filter(lambda item: len(item['audio']["array"])<(max_audio_len*16_000), num_proc=4)
                    loaded_datasets[k] = loaded_datasets[k].filter(lambda item: len(item['audio']["array"])>(min_audio_len*16_000), num_proc=4)
            if do_filter:
                loaded_datasets[k] = loaded_datasets[k].filter(functools.partial(filter_long_text, tokenizer=tokenizer, max_text_len=max_text_len), num_proc=4)   
